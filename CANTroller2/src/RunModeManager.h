...
private:
    ...
    void handleHoldMode() {
     ...
     void handleFlyMode() {
        ...
        if (ctrl == HOTRC && !(simulating && sim_joy) && !hotrc_radio_detected) updateMode(HOLD);  // Radio must be good to fly. This should already be handled elsewhere but another check can't hurt

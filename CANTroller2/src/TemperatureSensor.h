...
public:
    ...
    static const std::map<SensorType, TemperatureLimits> temp_limits_map;
...
public:
    TemperatureSensor(location location, const DeviceAddress& address, DallasSensor* tempsensebus, SensorType type, ErrorManager* errorManager)
   : _location(location), _address(address), _tempsensebus(tempsensebus), _temperature(-999), _type(type), _limits(temp_limits_map.at(type)), _errorManager(errorManager) {}
    ...
    // checks temperature against limits and handle warning/alarms as needed
    void check_temperature_limits() {
        float temp = get_temperature();
        if (temp > get_limits().get_alarm()) {
            _errorManager->handle_error(ErrorType::TEMPERATURE_ALARM, true);
        } else if (temp > get_limits().get_warning()) {
            _errorManager->handle_error(ErrorType::TEMPERATURE_WARNING, true);
        } else {
            _errorManager->handle_error(ErrorType::TEMPERATURE_ALARM, false);
            _errorManager->handle_error(ErrorType::TEMPERATURE_WARNING, false);
        }
    }
...
const std::map<TemperatureSensor::SensorType, TemperatureLimits> TemperatureSensor::temp_limits_map = {

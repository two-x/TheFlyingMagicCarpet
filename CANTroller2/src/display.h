...
static std::string datapage_names[datapages::NUM_DATAPAGES][disp_tuning_lines] = {
    ...
    { brAk"Posn", brAk"Mode", "Pn|PrErr", "BrakeTgt", "HybrdTgt", "OutRatio", "MotrHeat", __________, "Brake Kp", "Brake Ki", "Brake Kd", },  // PG_BPID
...
static std::string tuneunits[datapages::NUM_DATAPAGES][disp_tuning_lines] = {
    ...
    { "in",   scroll, "psin", "psin", "%",    "%",    degreF, ______, ______, "Hz",   "s",    },  // PG_BPID
...
static std::string unitmapnames[10] = { "usps", "us", "rpms", scroll, b1nary, "%", "ohm", "eyes", "psin", degree };  // unit strings matching these will get replaced by the corresponding bitmap graphic below
static constexpr uint8_t unitmaps[10][17] = {  // 17x7-pixel bitmaps for where units use symbols not present in the font, are longer than 3 characters, or are just special
  ...
  private:
    ...
    void disp_datapage_values() {
        ...
        if (datapage == PG_RUN) {
            ...
            draw_dynamic(14, brake.hybrid_out_ratio_pc, 0.0f, 100.0f);  // brake_spid_speedo_delta_adc, -range, range);
            draw_dynamic(15, brake.motorheat(), brake.motorheatmin(), brake.motorheatmax());  // brake_spid_speedo_delta_adc, -range, range);
            draw_eraseval(16);

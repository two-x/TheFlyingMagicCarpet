...
static constexpr uint8_t unitmaps[20][13] = {  // now 13x7-pixel bitmaps for unit strings. required when string is over 2 characters
    ...
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x7e, 0x3f, 0x00, 0x7e, 0x3f, 0x10, },  // scroll arrows - to indicate multiple choices. this one is right-aligned one char over to allow longer names
    ...
    { 0xfe, 0x12, 0x12, 0x0c, 0x00, 0x04, 0x7d, 0x00, 0x44, 0x28, 0x10, 0x28, 0x44, },  // pix
    ...
    // { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3e, 0x00, 0x1c, 0x22, 0x22, 0x1c, 0x00, },  // binary vertical on/off dots
    ...
    void edit_values(int rmode) {
        if (tunctrl == EDIT && idelta) {  // Change tunable values when editing
            if (datapage == PG_RUN) {
                ...
                else if (sel == 12) tune(&flashdemo);  //  neo->enable_flashdemo(flashdemo); }
                else if (sel == 13) neo->setbright(tune(neobright, 1, 100));  //  neo->setbright(neobright); }

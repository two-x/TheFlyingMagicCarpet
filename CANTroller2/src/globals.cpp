...
Timer::Timer(uint32_t arg_timeout_us) {
  set((int64_t)arg_timeout_us);
}

void Timer::set(int64_t arg_timeout_us) {
  timeout_us = arg_timeout_us;
  reset();
}

void Timer::set(uint32_t arg_timeout_us) {
  set((int64_t)arg_timeout_us);
}
...
float convert_units(float from_units, float convert_factor, bool invert, float in_offset, float out_offset) {
  ...
  printf("convert_units refused to divide by zero: %f, %f, %d, %f, %f", from_units, convert_factor, invert, in_offset, out_offset);
  return -1;
}
...
//  ---- tunable ----
float brake_pos_convert_in_per_adc = 3.3 * 10000.0 / (5.0 * adcrange_adc * 557);  // 3.3 v * 10k ohm / (5 v * 4095 adc * 557 ohm/in) = 0.0029 in/adc = 2.89 m-in/adc 
...
Servo gas_servo; // cant be static
Servo brake_servo; // cant be static
Servo steer_servo;// cant be static
Adafruit_NeoPixel neostrip(1, neopixel_pin, NEO_GRB + NEO_GRB + NEO_KHZ800); // cant be static

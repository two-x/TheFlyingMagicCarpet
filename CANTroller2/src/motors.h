...
static std::string motorctrlcard[NumMotorCtrls] = { "Disabl", "OpLoop", "ActPID", "Calib", "AuPID" };
static std::string motoractioncard[NumMotorActions] = { "Cruise", "Start", "Halt", "Manual", "Releas", "Park", "AuStop", "AuHold" };
static std::string cruiseschemecard[NumCruiseSchemes] = { "OnePul", "HoldTm" };
static std::string brakefeedbackcard[NumBrakeFB] = { "BkPosn", "BkPres", "Hybrid" };
static std::string openloopcard[NumOpenLoops] = { "Median", "AutRel", "ARHold" };
...
class ServoMotor {
  ...
  public:
    ...
    float pc[NumMotorVals] = { 0.0f, NAN, 100.0f, 0.0f, NAN, NAN, NAN, NAN, NAN }; // percent values [OpMin/Stop/OpMax/Out/Govern/AbsMin/AbsMax/Margin/Parked]  values range from -100% to 100% are all derived or auto-assigned
    float si[NumMotorVals] = { 45.0f, NAN, 90.0f, NAN, 0.0f, 180.0f, 1.0f, NAN }; // standard si-unit values [OpMin/Stop/OpMax/Out/Govern/AbsMin/AbsMax/Margin/Parked]
    float us[NumMotorVals] = { NAN, NAN, NAN, NAN, NAN, 500.0f, 2500.0f, NAN, NAN }; // us pulsewidth values [OpMin/Stop/OpMax/Out/Govern/AbsMin/AbsMax/Margin/Parked]
    ...
    virtual void set_ctrlmode(int a_mode) = 0; // each child must define this function
    virtual void set_action(int a_action, bool force=false) = 0; // each child must define this function
    virtual float out_pc_to_si(float _pc) {
        return map(_pc, pc[AbsMin], pc[AbsMax], si[AbsMin], si[AbsMax]); 
    }
    virtual float out_si_to_pc(float _si) {
        return map(_si, si[AbsMin], si[AbsMax], pc[AbsMin], pc[AbsMax]);
    }
    virtual float out_si_to_us(float _si, bool rev=false) { // works for motor with or without stop value
        return map(_si, si[AbsMin], si[AbsMax], rev ? us[AbsMax] : us[AbsMin], rev ? us[AbsMin] : us[AbsMax]);
    }
    virtual float out_pc_to_us(float _pc, bool rev=false) { // works for motor with or without stop value
        return map(_pc, pc[AbsMin], pc[AbsMax], rev ? us[AbsMax] : us[AbsMin], rev ? us[AbsMin] : us[AbsMax]);
    }
    virtual void derive() {} // calculate derived values when a variable is changed
...
class JagMotor : public ServoMotor {
  ...
  public:
    ...
    void derive() override { // calc pc and voltage op limits from volt and us abs limits 
    ...
    float out_pc_to_si(float _pc) override {
    ...
    float out_si_to_pc(float _si) override {
    ...
    float out_si_to_us(float _si, bool rev=false) override { // works for motor with center stop value
    ...
    float out_pc_to_us(float _pc, bool rev=false) override { // works for motor with center stop value
...
//    CtrlPID : Servo angle is determined by PID calculation designed to converge engine rpm to a target value set proportional to trigger pull
// cruise_adjust_scheme : (compile time option) Pick from different styles for adjustment of cruise setpoint. I prefer CruiseHoldTime.
//    CruiseOnePull : Cruise locks servo angle (throttle_target_pc), instead of pid. Moving trigger from center adjusts setpoint proportional to how far you push it before releasing (and reduced by an attenuation factor)
//    CruiseHoldTime : Cruise locks servo angle (throttle_target_pc), instead of pid. Any non-center trigger position continuously adjusts setpoint proportional to how far it's pulled over time (up to a specified maximum rate)
class ThrottleControl : public ServoMotor {
  ...
  public:
    ...
    // float idle_si[NumMotorVals] = { 58.0f, NAN, 65.0f, NAN, NAN, 0.0f, 180.0f, 1.0f, NAN }; // in angular degrees [OpMin(hot)/-/OpMax(cold)/Out/-/AbsMin/AbsMax/Margin/Parked]
    // float idletemp_f[NumMotorVals] = { 60.0f, NAN, 205.0f, 75.0f, NAN, 40.0f, 225.0f, 1.5f, NAN }; // in degrees F [OpMin/-/OpMax/Out/-/AbsMin/AbsMax/Margin/Parked]
    ...
    QPID pid, cruisepid;
    // bool pid_enabled, cruise_pid_default; // if servo higher pulsewidth turns ccw, then do reverse=true   // cruise_trigger_released = false; 
    bool just_started_cruise_session = true;
    int cruise_adjust_scheme = CruiseHoldTime;
    ...
    void derive() override { // calc derived limit values for all units based on tuned values for each motor
    ...
    void init_cruise_pid() {
        cruisepid.init(speedo->ptr(), cruise_outmin_ptr[gas_pid_default], cruise_outmax_ptr[gas_pid_default],
            cruise_kp[gas_pid_default], cruise_ki[gas_pid_default], cruise_kd[gas_pid_default],
    ...
    void setup(Hotrc* _hotrc, Speedometer* _speedo, Tachometer* _tach, Potentiometer* _pot, TemperatureSensorManager* _temp) {
        ...
        init_pid();
        init_cruise_pid();
        set_ctrlmode(run_motor_ctrlmode[runmode][_Throttle]);
        set_cruise_pid_default(cruise_pid_default);
        ...
        ezread.squintf("  cruise pid %sabled, w/ %s adj scheme\n", cruise_pid_default ? "en" : "dis", cruiseschemecard[cruise_adjust_scheme].c_str());
    }
  ...
  private:
    ...
    float cruise_adjust(int joydir, float thr_targ) {
        if (cruise_adjust_scheme == CruiseHoldTime) {  // this scheme continually adjusts the current thr_targ based on how far from center (in either dir) you hold the trigger
        ...
        else if (cruise_adjust_scheme == CruiseOnePull) { // this scheme makes a single adjustment with each departure from trigger center, based on how far you push it (in either dir).
    ...
    float cruise_logic(float thr_targ) {
        ...
        if (just_started_cruise_session) {                          // run once at start of each session to set cruise control values
            ...
            if (cruise_pid_default) cruisepid.set_output(thr_targ); // if this is the end of an adjustment, update pid out to current gas tgt
            if (cruise_pid_default) thr_targ = cruisepid.compute(); // if running on pid and not adjusting, use the pid math to drive thr_targ
        ...
        if (joydir == HrcCent) { // if joystick is at center, or this is the first loop of our session  //  || !cruise_trigger_released
            if (cruise_pid_default && cruise_adjusting) cruisepid.set_output(thr_targ);  // if this is the end of an adjustment, update pid out to current gas tgt
            ...
            if (cruise_pid_default) cruisepid.set_target(speedo->val()); // during adjustment, constantly update the pid speed target to the current speed
        }
        return thr_targ;
    }
    void set_output() {
        ...
        else new_out = rate_limiter(throttle_target_pc); // the pid should already be set to limit rate if needed.   pc[Out] = rate_limiter(new_out);  max_out_change_rate_pcps
  ...
  public:
    void set_ctrlmode(int a_mode) {
        if (a_mode == CtrlAutoPID) a_mode = gas_pid_default ? CtrlPID : CtrlOpenLoop; // translate "CtrlAutoPID" (a fake mode) into CtrlPID or CtrlOpenLoop mode, depending on current configuration
        if (a_mode != CtrlDisable && a_mode != CtrlOpenLoop && a_mode != CtrlPID && a_mode != CtrlCalibrate) return; // allow only valid modes
        if (a_mode == ctrlmode) return; // if mode isn't being changed, then do nothing and ditch out
        if (a_mode == CtrlCalibrate) {
            float temp = pot->mapToRange(0.0f, 180.0f);
            if (temp < si[Parked] || temp > si[OpMax]) {
                ezread.squintf("cal gas: first adjust pot to w/i valid op range\n");
                return; // do not change to cal mode if attempting to enter while pot is out of range
            }
        }
        else if (a_mode == CtrlPID || a_mode == CtrlOpenLoop) {
            gas_pid_default = (a_mode == CtrlPID);
            update_pid();
            update_cruise_pid();
        }
        cal_gasmode = (a_mode == CtrlCalibrate);
        ctrlmode = a_mode; // actually change the motor mode
    }
    void set_action(int a_action, bool force=false) {
        if (a_action != ActionCruise && a_action != ActionStarting && a_action != ActionHalt &&
        ...
        if (a_action == ActionCruise) { // upon entering cruise mode ...
            ...
            throttle_target_pc = (ctrlmode == CtrlPID) ? tach->val() : pc[Out]; // set throttle target to its current state, for a smooth transition into cruise mode
        }
        motoraction = a_action;
    }
    void update_pid() { // must call whenever governor or tach limits are changed
        if (gas_pid_default) pid.set_limits(&pc[OpMin], &pc[Govern]);
        pid.set_output(gas_pid_default ? tach->val() : pc[Out]); // set pid output to current value appropriate to current gas pid status
    }
    void set_cruise_tunings(float a_kp=NAN, float a_ki=NAN, float a_kd=NAN) { // for external tuner to calibrate the cruise pid coefficients. helper function needed to remember the edited settings independently for whether gas pid is open or closed loop
        if (!std::isnan(a_kp)) cruise_kp[gas_pid_default] = a_kp; // remember these calibrated settings as specific to the current state of the gas pid 
        if (!std::isnan(a_ki)) cruise_ki[gas_pid_default] = a_ki;
        if (!std::isnan(a_kd)) cruise_kd[gas_pid_default] = a_kd;
        cruisepid.set_tunings(cruise_kp[gas_pid_default], cruise_ki[gas_pid_default], cruise_kd[gas_pid_default]);
    }
    void update_cruise_pid(bool limits_only=false) { // always and only call whenever either pid enable is changed. send in true if only adjusting limits
        if (!cruise_pid_default) return;
        cruisepid.set_limits(cruise_outmin_ptr[gas_pid_default], cruise_outmax_ptr[gas_pid_default]); // switch cruise pid limits to those appropriate for current gas pid status
        if (limits_only) return;
        cruisepid.set_target(speedo->val()); // set pid loop speed target to current speed
        cruisepid.set_output(gas_pid_default ? tach->val() : pc[Out]); // set pid output to current value appropriate to current gas pid status
        cruisepid.set_tunings(cruise_kp[gas_pid_default], cruise_ki[gas_pid_default], cruise_kd[gas_pid_default]);
    }
    void set_cruise_pid_default(bool new_cruise_pid_ena) {
        if (new_cruise_pid_ena != cruise_pid_default) update_cruise_pid();
        cruise_pid_default = new_cruise_pid_ena;
    }
    void set_cruise_scheme(int newscheme) {
        cruise_adjust_scheme = newscheme;
    }
...
// Notes on tuning brake coefficients:
...
class BrakeControl : public JagMotor {
  ...
  public:
    ...
    bool enforce_positional_limits = true, no_feedback = false; // default for use of pid allowed
    ...
    QPID* pid_dom = &(pids[FBPosition]); // AnalogSensor sensed[2];
    ...
    void setup(Hotrc* _hotrc, Speedometer* _speedo, CarBattery* _batt, PressureSensor* _pressure, BrakePositionSensor* _brkpos, ThrottleControl* _throttle, TemperatureSensorManager* _tempsens) {  // (int8_t _motor_pin, int8_t _press_pin, int8_t _posn_pin)
        ...
        derive();
        pids[FBPressure].init(pressure->ptr(), &(pc[OpMin]), &(pc[OpMax]), press_kp, press_ki, press_kd, QPID::pmod::onerr,
        ...
        pids[FBPosition].init(brkpos->ptr(), &(pc[OpMin]), &(pc[OpMax]), posn_kp, posn_ki, posn_kd, QPID::pmod::onerr, 
        ...
        set_ctrlmode(run_motor_ctrlmode[runmode][_BrakeMotor]);
        set_feedback(feedback);
        set_openmode(openloop_mode);
    }
    void update() {
        if (runmode == LowPower) return;
        ...
        if (pid_timer.expireset()) {
            ...
            float new_out = set_output();             // determine motor output percent value
            pc[Out] = postprocess_out(new_out);       // fix motor pc value if it's out of range or threatening to exceed positional limits
    ...
    void derive() override { // to-do below: need stop/hold values for posn only operation!
        JagMotor::derive();
    ...
    void set_out_changerate_pcps(float newrate) {
        ...
        for (int mypid=FBPosition; mypid<=FBPressure; mypid++) pids[mypid].set_max_out_changerate(max_out_changerate_pcps);
    }
  ...
  private:
            ...
            if (std::isnan(motor_heat)) {
                ...
                if (overtemp_shutoff_brake) { // here the brakemotor is shut off if overtemp. also in diag class the engine is stopped
                    if (motor_heat > tempsens->opmax(loc::TempBrake)) {
                        ...
                        printed_error = true;
                        // set_ctrlmode(ActionHalt, false); // commented b/c is done in diag (it didn't seem to work here)       // stop the brake motor // pc[Out] = pc[Stop];  // set_ctrlmode(ActionPark, false);
    ...
    void set_dominant_sensor() {
        ...
        else dominantsens = (hybrid_ratio_pc >= 50.0f) ? FBPressure : FBPosition; // round to 0 or 1, the indices of posn and pres respectively
        pid_dom = &(pids[dominantsens]);
        posn_pid_active = (dominantsens == FBPosition); // for display
    }
    ...
    float calc_hybrid_ratio_pc(float pressure_val) { // returns pressure influence as a ratio (from 0.0 to 1.0) for a given pressure level in percent 
        // if (feedback == FBNone) return NAN;
        if (feedback == FBPressure) return 100.0f;
        if (feedback == FBPosition) return 0.0f;
        if (std::isnan(pressure_val)) return NAN;
    ...
    void calc_combined_brake() {  // calculates and saves combined brake value percent, based on readings of the feedback sensors. harmless to call even if running openloop
        hybrid_ratio_pc = calc_hybrid_ratio_pc(pressure->pc());
        // if (feedback == FBNone) {
        //     combined_read_pc = NAN;
        //     return;
        // }
        set_dominant_sensor(); // this is for idiot light display
    ...
    float calc_open_out() {
        ...
        if (openloop_mode == OpenAutoRelHoldable) { // OpenAutoRelHoldable: releases brake whenever trigger is released, and presses brake when trigger pulled to max. anywhere in between stops brake movement
    ...
    void set_target(float targ_pc) { // sets brake target percent. if hybrid, will set pressue and posn targets per current hybrid ratio, and set both pid targets in case pids are used
        target_pc = targ_pc;
        // if (feedback == FBNone) return; // target_pc is overall, (or desired or combined ?) value is used if running openloop
        target_si[FBPressure] = pressure->from_pc(target_pc); // feed this into pid if enabled, (otherwise could use as a motor cutoff threshold? this mode not implemented)
        target_si[FBPosition] = brkpos->from_pc(target_pc); // (saved into variables for display)
        for (int mypid=FBPosition; mypid<=FBPressure; mypid++) pids[mypid].set_target(target_si[mypid]);  // feed target vals to pid loops. this is harmless if pids disabled, it will have no effect
    }
    // TODO review logic of AutoStop/AutoHold actions and carstop() code against runmode code
    void carstop(bool panic_support=true) { // autogenerates motor target values with the goal of stopping the car
        ...
        bool autostopping_last = autostopping;
        ...
        else if (allow_openloop_autobrake || (panic && allow_openloop_panicstop)) {
            autostopping = !blindaction_timer.expired(); // to panic stop without any feedback, we just push as hard as we can for a couple seconds then stop
            if (autostopping) pc[Out] = pc[OpMax]; // note this is dangerous in that it could mechanically break stuff. if running open loop avoid panicstops
            else if (autostopping_last) pc[Out] = pc[Stop];
        }
    ...
    float set_output() { // returns updated motor output % in whatever way is right for the current ctrlmode and action
        ...
        //     ezread.squintf("brake: invalid ctrlmode=%d, set to %s\n", ctrlmode, motorctrlcard[ctrlmode].c_str());
        // }
        if (motoraction == ActionAutoHold) { // AutoHold initially stops the car (if moving) or apply decent brake pressure (if not), then thereafter ensure it stays stopped
            if (ctrlmode != CtrlPID) {
                ezread.squintf(ezread.madcolor, "brake: can't %s in %s mode\n", motoractioncard[motoraction].c_str(), motorctrlcard[ctrlmode].c_str());
                set_action(ActionHalt);
                return pc[Out]; // output unchanged
            }
        ...
        else if (motoraction == ActionAutoStop) { // AutoStop tries to stop the car by closing throttle and applying steadily increasing brakes. After stopped or timeout, changes ctrlmode to Halt
  ...
  public:
    void set_ctrlmode(int a_mode) { // external_request set to false when calling from inside the class (to remember what the outside world wants the mode to be)   
        if (a_mode == CtrlAutoPID) a_mode = brake_pid_default ? CtrlPID : CtrlOpenLoop; // translate "CtrlAutoPID" (a fake mode) into CtrlPID or CtrlOpenLoop mode, depending on current configuration
        if (a_mode != CtrlDisable && a_mode != CtrlOpenLoop && a_mode != CtrlPID && a_mode != CtrlCalibrate) return; // allow only valid modes
        if (a_mode == ctrlmode) return;
        if (a_mode == CtrlPID || a_mode == CtrlOpenLoop) {
            brake_pid_default = (a_mode == CtrlPID);
            set_action(motoraction, true); // force set action to current action, in order to restart it using the new mode
            // update_pid(FBPosition); // TODO need to write this!  similar to throttle class
            // update_pid(FBPressure); // TODO need to write this!  similar to throttle class
        }
        reset_state();
    ...
    void set_action(int a_action, bool force=false) { // use force=true when changing ctrlmode and need to restart action
        ...
        if (ctrlmode == CtrlOpenLoop) {
            if (a_action == ActionAutoStop && panicstop && allow_openloop_panicstop) drop_to_manual = false;
            else if (!allow_openloop_autobrake &&
                (a_action == ActionRelease || a_action == ActionPark || 
                a_action == ActionAutoHold || a_action == ActionAutoStop))
                    drop_to_manual = true;
        }
        if (drop_to_manual) {
    ...
    void set_feedback(int a_fb) {
        a_fb = constrain(a_fb, 0, NumBrakeFB - 1); // constrain in case UI acceleration may shoot past out of range
        // if (ctrlmode == CtrlPID && a_fb == FBNone) return; // instead of previous: // if (feedback == FBNone) brake_pid_default = false;
        feedback_enabled[FBPosition] = (a_fb == FBPosition || a_fb == FBHybrid); // set sensor enable consistent with feedback config
        feedback_enabled[FBPressure] = (a_fb == FBPressure || a_fb == FBHybrid); // set sensor enable consistent with feedback config
    ...
    bool parked() {
        ...
        if (feedback_enabled[FBPressure]) return pressure->parked();
        ezread.squintf(ezread.sadcolor, "warn: brake unaware if parked w/o sensors\n");
        return false;
    }
    bool released() {
        if (feedback_enabled[FBPosition]) return brkpos->released();
        if (feedback_enabled[FBPressure]) return pressure->released();
...
class SteeringControl : public JagMotor {
  public:
    ...
    void set_ctrlmode(int a_mode) {
        if (a_mode != CtrlDisable && a_mode != CtrlOpenLoop) return; // allow only valid modes
        ctrlmode = a_mode;
    }
    void set_action(int a_action, bool force=false) {
        if (a_action != ActionHalt && a_action != ActionManual) return; // allow only valid modes
        motoraction = a_action;
    }
  private:

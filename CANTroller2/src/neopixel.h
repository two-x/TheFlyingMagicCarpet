...
class IdiotLights {
  public:
    ...
    bool* vals[iconcount] = {
        ...
        &car_hasnt_moved, &starter.req_active, &starter.motor, &fuelpump.status_inverse, // &powering_up,
        &brake.posn_pid_active, &bootbutton.now, &nowtouch, tach.pin_activity_ptr(), &encoder.enc_a, &web_disabled, 
        &running_on_devboard, &sensidiots[_GasServo], &sensidiots[_BrakeMotor], &sensidiots[_SteerMotor],
    ...
    uint8_t icon[iconcount][11] = {
        ...
        { 0x4c, 0x2a, 0x19, 0x00, 0x63, 0x63, 0x63, 0x63, 0x77, 0x3e, 0x1c, },  // magnet w/ zap
    ...
    std::string letters[iconcount] = {
        "SL", "SR", "\xf7""E", "\xf7""W", "RC", "P\x13", "SI", "Pk", "AS", "AH", "Aj",  // 11 per row 
        "HM", "Rq", "St",      "FP",      "Pn", "Bt",    "NT", "Mg", "eA", "WD", "Dv",

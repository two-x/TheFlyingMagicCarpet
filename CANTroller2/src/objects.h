...
void maf_task(void *parameter) {
    while (!bootup_complete) vTaskDelay(pdMS_TO_TICKS(10)); // let boot finish before reading
    while (true) {
        ...
        lightbox.update(speedo.val());  // re-enabled: the correlation with mapsens breaking was the old dual-i2c-driver-stack issue (touch vs Wire),
                                         // now fixed - see I2C::_busmutex / LightingBox::update() comments
  ...
  public:
    ...
    void update() {  // starter drive handler logic.  Outside code interacts with handler by calling request(XX) = ReqOff, ReqOn, or ReqTog
        ...
        if (brake_before_starting && !simple_brake) {        // if we must apply brakes before starting
            ...
            else if (brake.motoraction != ActionAutoHold) {  // if we haven't yet told the brake to hold down

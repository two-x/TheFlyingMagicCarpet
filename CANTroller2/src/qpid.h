...
class IdleControl {  // Soren - To allow creative control of PID targets in case your engine idle problems need that.
  ...
  public:
    IdleControl (float* target_rpm, float* measraw_rpm, float* measfilt_rpm, float* coolant_f,  // Variable references: idle target, rpm raw, rpm filt, Engine temp
                 float idlehigh_rpm, float idlehot_rpm, float idlecold_rpm,  // Values for: high-idle rpm (will not stall), hot idle nominal rpm, cold idle nominal rpm 
                 float tempcold_f, float temphot_f,  // Values for: operational temp cold (min) and temp hot (max) in degrees-f
                 uint32_t settletime_us = 2000000,  // Period over which the idle will be lowered from high-idle to final idle
                 idlemodes idlemode = idlemodes::softlanding) {  // Configure idle control to just soft land or also attempt to minimize idle
        // target_rpm = Target_rpm;      measraw_rpm = Measraw_rpm; measfilt_rpm = Measfilt_rpm; coolant_f = Coolanttemp_f;
        // idlehigh_rpm = Idlehigh_rpm;  idlehot_rpm = Idlehot_rpm; idlecold_rpm = Idlecold_rpm;
        // tempcold_f = Tempcold_f;      temphot_f = Temphot_f;     idlemode = Idlemode;
        calc_idlespeed();
        targetlast_rpm = *target_rpm;
        setTimer.set ((int64_t)Settime_us);  // Time period over which the idle will subsequently drop further to its final target value

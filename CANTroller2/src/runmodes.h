  ...
  public:
    ...
    int mode_logic() {
        if (mode != LOWPOWER && mode != CAL) {
            if (in_basicmode) mode = BASIC;  // if basicmode switch on --> Basic Mode
  ...
  private:
    ...
    void run_basicMode() { // Basic mode is for when we want to operate the pedals manually. All PIDs stop, only steering still works.
        if (we_just_switched_modes) {
            ...
            powering_up = false;  // to cover unlikely edge case where basic mode switch is enabled during wakeup from lowpower mode
            watchdog.set_codestatus(Parked);
        }
        if (hotrc.sw_event(CH4) && !ignition.signal) mode = LOWPOWER;
        if (!in_basicmode && !tach.stopped()) mode = speedo.stopped() ? HOLD : FLY;  // If we turned off the basic mode switch with engine running, change modes. If engine is not running, we'll end up in Stall Mode automatically
    ...
    void run_lowpowerMode() {  // turns off syspower and just idles. sleep_request are handled here or in standby mode below
        ...
        if (powering_up && pwrup_timer.expired()) mode = (in_basicmode) ? BASIC : STANDBY;  // finish powering up . display->all_dirty();  // tells display to redraw everything. display must set back to false
        sleep_request = REQ_NA;
    }
    void run_standbyMode() { // In standby mode we stop the car if it's moving, park the motors, go idle for a while and eventually sleep.
        if (we_just_switched_modes) {              
            ...
            powering_up = calmode_request = autosaver_request = REQ_OFF;

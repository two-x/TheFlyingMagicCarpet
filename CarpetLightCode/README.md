...
- **Short press**: cycles to the next light show (Nightrider → Flame → Equalizer → SpeedStripes → Lighthouse → ...). Persisted to flash immediately.
...
- **Rotation**: each beam's angular velocity (range ±360°/s = ±1Hz) is its own independent random walk (`LighthouseShow::RandomWalk`) — smoothly ramps toward a freshly-randomized target (±10°/s step) roughly every second, so it can drift through zero and reverse direction over time, per request.
...
## Flame show variations (`FlameShow.h`)
...
- The pot does **two jobs at once** in variations 2/3: it still controls the sparkle/fire-sim cadence (via the `delay(potval)` at the bottom of `update()`, unchanged), **and** it scales the max hue-drift rate — fully down = 50% of the base max (a full spectrum cycle in as little as 40s instead of 20s), fully up = 100%. Direction and the random-walk mechanism are unchanged, only the ceiling it wanders within shifts.
...
**`SpeedStripesShow`**: the two long side rope strips are the only fixtures that physically run front-to-back along the car's 16ft length (the front/back rope edges don't, and stay off while moving — see "Perimeter rope lights" above). Each side strip (352 LEDs) is divided into 4 bands of 88 LEDs each — a quarter of the car's length (4ft) — that alternate lit/dark with a smoothed (not hard-edged) transition between them. The pattern continuously scrolls from front toward back at a rate proportional to the live speed reading (`LEDS_PER_MPH_PER_SEC` in the show, a to-taste constant), giving a sense of forward motion; at a standstill, or if the telemetry link is stale, the front-to-back scroll is replaced by the spin described below.

**Color**: rather than a single fixed hue, every lit band samples a position on a 4-color palette (`colorPalette_`, built with FastLED's `CRGBPalette16(c1,c2,c3,c4)` so the 4 colors blend smoothly into each other across the palette) mapped across the same period as the on/off band envelope — so color continuously flows as the pattern moves, in both moving and stopped mode, rather than the whole show being one static hue. The 4 colors are freshly randomized (`random8()`-picked hues) about every 20 seconds. The same slow desaturation cycle as `EqualizerShow` (100%→85%→100% saturation over a 30-second sine) breathes on top.

**Performance note**: the Due's SAM3X8E is a Cortex-M3 with **no hardware FPU**, so `sinf()`/`tanhf()`/`atan2f()`/`cosf()` are all expensive software-emulated calls — and this show's own `update()` keeps running in the background even while a config screen is on-screen (`currLightShow->update()` is called unconditionally every loop, "invisibly," so the show doesn't jump on cancel). An earlier version called `atan2f()` per rope LED per frame in `renderSpin()` (1016×/frame) and recomputed the desaturation `cosf()` per LED instead of once per frame — expensive enough to be felt as general UI choppiness elsewhere in the system, not just in this show. Fixed by: caching `ropeAngleDeg()`/`chinaAngleDeg()` once in `start()` (`cacheAngles()`, pure static geometry — never needs recomputing per-frame) instead of every `renderSpin()` call; computing `currentSatFraction()` once per `update()` call and passing it down instead of recomputing per LED; and replacing the `sinf()+tanhf()` brightness envelope with FastLED's table-based `sin8()` plus a cheap integer contrast boost.

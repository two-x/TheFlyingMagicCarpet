...
- **Variation 2** (pixel_war): a simpler "territory war" — the rope's colored boundary is pushed by each qualifying bass hit (an immediate impulse, partially given back ~150ms later), while turns trade between the 2 colors every `cycleBeats_` hits. Megabars/china flash in bass/treble pools.

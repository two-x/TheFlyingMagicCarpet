...
A self-contained single-file HTML/JS/Canvas simulator, built so a remote developer can debug firmware, add shows, and tune existing ones from `src/*.h` alone — no physical car needed. **Not an independent creative sandbox**: its light-show logic is a faithful JS port of `src/*.h` (`AudioBoard.h`'s math, `NightriderShow`/`FlameShow`/`EqualizerShow`/`SpeedStripesShow`/`LighthouseShow`, config-mode navigation) and must stay that way — new show *behavior* belongs in firmware first, with the visualizer just running the same logic so the screen always matches the real carpet. Legitimate divergence is limited to simulating things *external* to firmware — audio input (mic/tab/file/synthetic tone standing in for the real MSGEQ7 chip), the vehicle speed SpeedLink would deliver, and the pot/encoder/button inputs themselves — never the show logic that consumes them. Renders a schematic top-down view with every fixture (rope, megabars, china) responding as on real hardware.

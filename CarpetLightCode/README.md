...
Only **4 continuous physical strips** exist. `NEO0_OFFSET`..`NEO3_OFFSET` (reading from `ropeLeds[]`) are the ones in use; `NEO4_OFFSET`..`NEO7_OFFSET` are leftover, unused offsets from a never-wired 8-strip plan.

**Bugfix ("whole segments of the neopixels are simply never lit" on the left side)**: `NEO3_OFFSET` (Left's logical start offset) was computed as if Rear — the strand immediately before Left in the Front→Right→Rear→Left loop — were `SIZEOF_LARGE_NEO`, a leftover assumption from the old 8-lane topology this project no longer has; Rear is actually `SIZEOF_SMALL_NEO`, same as Front. That put Left's local index 0 exactly `SIZEOF_LARGE_NEO - SIZEOF_SMALL_NEO` (196) pixels too far into the array, so every show that addresses rope pixels by side/Y position (`SpeedStripesShow`'s `renderSide()`/`renderZebraSide()`, plus the `NEO3_OFFSET` clear loops in `NightriderShow::start()`/`BumpingAudioShow::start()`) silently orphaned the first 196 real Left-strand pixels — never addressed by any local index — while wrapping its last 196 local indices around onto Front's own raw range instead. `NEO0`/`NEO1`/`NEO2_OFFSET` were unaffected (their own true starts don't depend on Rear's size).
...
- **Encoder rotation**: per-show, selects that show's own variation (not global) — e.g. `NightriderShow` has 2 (pot picks a live hue pair, vs. hue auto-cycling at a pot-controlled rate), `FlameShow` has 2 (below), `EqualizerShow` has 3 (below). Each show's variation persists per-show, so switching shows and back recalls where you left it. Selecting a show or variation prints an abbreviated console line (`showName()`, each show's own `LightShow::variationName()` — see `LightShow.h`), e.g. `show:FlameSparkle` or `FlameSparkle:flames`.
- **PressLeft / PressRight** (`LedController.h`'s `PushButton::handleRotationDuringHold()`): rotating the encoder WHILE its button is held cancels whatever press-tier was in progress (short/medium/long/extra-long/double all voided for the rest of that hold) and fires this instead, immediately, repeatably for further rotation in the same hold. No default action anymore (the peak-threshold adjustment that used to live here moved to PressPot, below) — a show may still act on it via `LightShow::onPressLeft()`/`onPressRight()`, but none currently do, so by itself this currently does nothing.
- **PressPot** (`LedController.h`'s `PushButton::updatePressPot()`/`PressPotIsActive()`/`PressPotValue()`): a distinct mechanism from PressLeft/PressRight — becomes active the instant the button is held, the hold hasn't yet reached the extra-long threshold, and the pot has moved more than 2% from wherever it was when the button went down; once active it stays active (regardless of further pot movement) until release, and — like PressLeft/PressRight — cancels whatever press-tier was in progress. `PressPotIsActive()`/`PressPotValue()` (0-100%, `NAN` when inactive) are available to any accessor (show or config code), not gated behind an override hook. Default (normal mode only): while active, sets the committed peak threshold directly to the live pot value, applied live and flash-saved on release (debounced further as a defensive backup — see "Nonvolatile flash writing" below).
...
- **Minimum beam separation**: the 2 beams' rotation velocities are independent random walks and can share sign (both drifting clockwise, or both counterclockwise), so nothing stops them drifting arbitrarily close together on their own. A per-frame enforcement keeps the 2 beams' guaranteed cores from ever touching (pushing the 2 angles apart symmetrically, 50/50, whenever they'd close past the minimum) — this position correction is a hard, exact snap every frame it triggers (never a soft velocity restriction; needed to keep the megabar guarantee genuinely unconditional), so one beam's guaranteed megabar coverage can never be contested/stolen by the other beam's winner-take-all comparison.
  - **Bugfix ("beams go the same speed in the same direction" for extended stretches)**: with only ~20° of free room between the 70°-minimum-separation boundary and the max possible 90° separation, and each beam able to random-walk up to 360°/s, the 2 beams typically closed that 20° in well under a second and then stayed pinned — the hard position snap above absorbed 100% of any further net-closing velocity every single frame, forcing the pair's *effective* angular rate to equal their shared average for as long as that tendency persisted (which, since the raw walk itself only drifts ±10°/s per ~1s tick, could be many seconds). Fixed by injecting a real, decaying "bounce" velocity bias on every collision (1.5s exponential decay, 40°/s kick) on top of the raw walks, so the pair genuinely separates again afterward instead of staying rigidly coupled — the hard position snap itself is untouched, so the megabar guarantee stays exactly as unconditional as before.
  - **Follow-up bugfix ("both beams always end up going counterclockwise")**: the bounce kick above was itself re-applied every single frame the collision condition stayed true, not once per collision — a single 40°/s kick is small next to the beams' combined closing rate (up to 720°/s), so it typically took many consecutive frames to actually separate, and since the kick's sign stays constant for as long as the same 2 beams keep approaching from the same side, those repeated kicks all landed in the same direction each frame, growing essentially unbounded (a real per-frame impulse vs. a decay rate ~180x slower) until the accumulated bias dwarfed the raw random-walked velocities and dictated the beams' apparent rotation itself. Fixed by making the kick edge-triggered — a real single bounce impulse on the frame separation first drops under the minimum, not a ratcheting force re-applied on every frame the beams happen to still be touching it.
...
- **Rope**: same falloff/winner-take-all as megabars, evaluated at each of ~1016 cached LED angular positions instead of 12 fixed ones — but 25% narrower (`ROPE_CONE_HALF_WIDTH_DEG`/`ROPE_CONE_FADE_DEG`, per request), since rope has no megabar-style coverage guarantee to protect. Megabars/china keep the full guaranteed width.
...
The rope runs a classic Fire2012-style heat simulation (cool → disperse → spark → assign-color-from-palette). 2 variations, cycled by encoder rotation (shifting hues/hue to white have been removed):

- **Variation 0/1** ("waterflames"/"flames"): the two fixed 256-entry palettes (`DarkBlue→Blue→Aqua→White` / `DarkRed→Red→Orange→Yellow`).
- The **shared global energy setting** controls sparkle/fire-sim cadence (`delay(potval)`, inverted so 100% energy = fastest).
...
**Idle (no-sound) base color**: outside `HIT_REACTIVE_WINDOW_MS` (750ms since the last real bass/treble hit — was 3s, shortened per request so shows react sooner after things go quiet), each floodlight sits at one of the palette's two reference colors (`FLOOD_LO_INDEX`/`FLOOD_HI_INDEX`) — re-rolled independently on each of that fixture's own sparks, still a real coin flip (not a fixed population split), but weighted **65% toward the dim/base color, 35% toward the bright one** (`FlameShow::pickBaseColor()`, `BASE_COLOR_PICK_PERCENT`), per request — was an even 50/50 flip.

## Equalizer show variations

3 variations, cycled by encoder rotation, defaulting to new_standard on boot (Chase and sub_standard have both been removed). All three react to `AudioBoard`'s shared **hit** value (`getBandHitPercent(Band)`/`getBandHitNonzero(Band)`, see "Audio processing" below) instead of each rolling its own peak-hold logic — decay behavior is uniform and tunable from Audio config subsetting 2b.

- **Variation 0** (VU meter): a dual VU meter across the whole front/rear rather than one corner. Bass is based on the *entire rear edge* (always lit) and grows forward along both sides as it rises (red); treble is based on the entire front edge and grows rearward (blue) — same red=bass/blue=treble convention, both driven by hit. `RIGHT`/`LEFT` are the exact midpoints of each side run, so each meter's 100%-reach is 15% of the true-corner-to-center half-length past that midpoint — maxed bass/treble simultaneously cross near the car's middle, blending 50/50 where they overlap. Brightness scales with level, not just reach — a quiet meter is dim, not just short. Megabars/china join the same meter, split by each fixture's own real front/rear position.
  ...
  - **Pot**: live-adjusts the shared peak threshold (`PkThresh`, 0-100% full pot, prints `PkThresh:<percent>%` 1s after settling) — soft takeover. Deliberately *not* part of the global energy group (a threshold isn't "energy"); unlike that group it *is* tied to a persisted value — leaving the show reverts it to whatever's saved, so casual tweaking never silently overwrites the committed setting.
- **Variation 1** (new_standard, boot default): a whole-show 2-color palette (`Bcolor`/`Tcolor`) that continuously drifts hue (10s red→yellow, 10s back) and saturation (75-100% over a 14s cycle). Rope: a bouncing 2-segment animation (a "big" segment per long side, a "little" one per short side, each independently reflecting off its own end), colored `Bcolor`/`Tcolor` (matches the floodlight color scheme, per explicit request — not a separate fixed palette). China/megabars each independently cycle between a handful of bass/treble patterns, advancing on a random 2-4-bass-hit schedule (`OptionCycler`), cross-fading 200ms on each change.
- **Variation 2** (pixel_war): a simpler "territory war" — the rope's colored boundary is pushed by each qualifying bass hit (an immediate impulse, partially given back ~150ms later), while turns trade between the 2 colors every `cycleBeats_` hits. Megabars flash in bass/treble pools (2 bass picks per hit, 1 treble). China is paired with ONE of its two real angular neighbors (either direction, randomly) rather than a fixed grouping — sometimes that's the 2 chinas sharing a corner bracket, sometimes the 2 aimed along the same edge (e.g. the 2 rear-aimed "back" chinas). Every `pwChinaTrebleEveryHits_` bass hits (itself re-randomized to a fresh 1 or 2 on each reroll), `pwRerollChinaTreble_()` picks a random china and a random neighbor direction; that pair and its real diagonal opposite (4 positions away around the car) go treble, the other 2 pairs stay bass — always exactly 2 pairs treble/2 bass. Each pair's color is fixed to its own band (not the war's turn-dependent color), brightness tracking that band's own live hit level.
- **Pot** (new_standard/pixel_war): sets the effects-cycle length in bass beats (`cycleBeats_`, 2-128, log-spaced, 15 discrete steps) — shared between the two.

**Triple-strobe** (any variation): toggled by double press while Equalizer is active — nonvolatile. When enabled, a qualifying bass hit (`AudioBoard::getBandHitNonzero(BandBass)`) flashes all 8 china and the 8 corner-adjacent megabars (all except headlight/left/rear/right cardinals, index `0`/`3`/`6`/`9`) full white for 3 pulses, 30ms on/20ms gap.
...
**Slow desaturation cycle** (VU meter): base colors (`bassClr`/`trebleClr`) drift saturation 100%→85%→100% over a smooth 30s sine (`currentSatFraction()`) — a subtle "breathing." `desaturate()` holds hue/value fixed, raising only the minimum channel — exact for these fully-saturated colors, a no-op on anything already less saturated. (new_standard/pixel_war run their own independent hue/sat drift instead, see above.)
...
- **Audio foresight** (0-700ms, default 340, subsetting 2c): the DSP pipeline "sees" a waveform before it reaches the speakers — this is how far `lightsShowTime` trails `audioReadTime`, i.e. how far ahead of the audible output the freshest capture currently sits. 0ms uses the freshest sample, keeping the DSP's full predictive lead; dialing up trades lead for tighter sync with true acoustic output.
...
- **Silence detection**: its own dedicated, much shorter timer (`SILENCE_DETECT_MS`, 750ms), fully decoupled from `AGC_WINDOW_MS` (4s) — `lastSoundMs_` tracks the timestamp of the most recent poll where any bin cleared its own noise floor (reusing `updateBandLevels()`'s existing `anyAboveFloor` check, computed there for an unrelated reason), and `isSilent()` reports true once `SILENCE_DETECT_MS` has passed since then. **Bugfix ("shows that change light behavior after silence is detected should do it sooner")**: silence detection used to piggyback on the same trailing `AGC_WINDOW_MS` window AGC/RMS scan (silent iff nothing anywhere in that whole 4s window had been above 0) — correct for what that buffer is tuned for (AGC boost smoothing genuinely needs that much depth), but a poor fit for "has sound just stopped," which real listeners judge in well under a second; every show gated on `isSilent()` (`EqualizerShow`'s "no sound" flood pattern for all 3 variations, `SpeedStripesShow`'s zebra china brightness) inherited that same ~4s lag. `AGC_WINDOW_MS`/the AGC boost math itself are completely untouched. Internal state only — read via `AudioBoard::isSilent()`, not a public field.
...
- **Peak threshold** (default **68%**) is the shared "this counts as a hit" level — see Configuration mode above. Auto-peak can raise the *effective* threshold above this per-bin, per-instant; never lowers it. Also adjustable live in normal show mode via **PressPot** — see "Encoder and button UI" above.
...
**`VarZebra`** (2nd variation): deterministic rainbow-hue stripes with an optional periodic black-stripe overlay, pot-driven. Below 2% pot (a dead zone matching this codebase's own real-world pot-noise assumption, `Potentiometer::NOISE_FLOOR_RAW`), black-stripe width is a flat, guaranteed 0 — no stripes, immune to pot jitter right at the minimum, not just usually-near-zero (fixes a real "flickering to black when pot is at minimum" bug: the old formula jumped straight from 0 to a fixed 0.5ft floor at the slightest reading above 0%). Above that dead zone, width ramps continuously (no jump at the boundary) up to 40ft at 100% pot. The colored gap between consecutive black stripes is always exactly 3x the black width, so the occurrence period (one black stripe's start to the next) is always exactly 4x the black width — e.g. a 2ft-wide stripe recurs every 8ft. Wider stripes therefore also come proportionally less often, rather than width and spacing scaling independently.

China's sound-reactive brightness (`zebraChinaBrightnessFraction()`) jumps to full global max after `ZEBRA_CHINA_SILENCE_MS` of continuous silence (750ms — was 10s, shortened per request), otherwise sits at `ZEBRA_CHINA_REST_FRACTION` of max except bass hits pushing it up to their own level.
...
| --- | --- |
| Megabars `[11,0,1]` (front 3, incl. headlight) | 4ft ahead of **each fixture's own real Y** — a preview, well before the stripe arrives. Not a single shared "front corner" position: `[11]`/`[1]` sit at a different real Y than `[0]` (the headlight), so each samples its own +4ft, not the headlight's (see BUGFIX in the class comment — this used to share one value across all 3) |
| China `[1,2]` (aimed along the front edge) | 1ft ahead of **each fixture's own real Y** — picks the same stripe up just as it's about to cross onto the carpet |
| Megabars `[5,6,7]` (rear 3) | 4ft behind **each fixture's own real Y** — mirrors the front, for the stripe that just left |
| China `[5,6]` (aimed along the rear edge) | 1ft behind **each fixture's own real Y** — mirrors the front |
...
**Stopped mode**: at speed 0, or when `SpeedLink` hasn't heard from the vehicle in 4s (stricter than `isFresh()`'s 2s default), the pattern **freezes** — `scrollOffset_` stops advancing, so every fixture (all deriving position from it) holds exactly where it was. After a further **10s** stopped, each stripe's hue pair starts slowly meandering — one shared drifting offset (`meanderHue_`, `RandomWalk`-based, same technique as `LighthouseShow`) adds on top of each stripe's hash-derived base hue, so they evolve together while staying distinct. A fresh stop always restarts this from scratch.
...
## Nonvolatile flash writing

The Due has no real EEPROM (see `Nvm.h`'s own file comment) — persisted settings ride on `DueFlashStorage`, which has finite write endurance and a real, measurable write cost. Two rules apply everywhere a value gets written to flash:

- **Read the currently-flashed value before writing, and skip the write if it's already equal.** Every `Nvm::saveXxx()` call site in `CarpetLightLogic.cpp` checks its matching `Nvm::loadedXxx()` first (cheap — `loadedXxx()` just returns from the RAM-resident `State` struct `load()` populated at boot, never a real flash read) and only calls `saveXxx()` if the value actually changed. This catches the case where a value is nudged away and back before being committed (e.g. dial a setting up then back down to its already-saved value before double-pressing to commit) — a real, avoidable write otherwise.
- **Debounce anything driven by a live pot/encoder twist**, so a continuous adjustment doesn't write on every tick. Most settings are already safe by construction — they only ever save once, on an explicit commit gesture (double press) — so the equality check above is their only real protection. The one setting that used to save *continuously* during live adjustment is peak threshold via PressPot (see "Encoder and button UI" above): `peakThreshSaveDebounce_`/`peakThreshSaveDirty_` (`CarpetLightLogic.cpp`) mark the value dirty and apply it live to `AudioBoard` immediately, but the actual flash write waits — primarily for PressPot to go *inactive* (release is a clean, precise "definitely done adjusting" signal, no arbitrary delay needed), with the debounce timer itself kept only as a defensive backup for the pathological case of a hold that never releases.

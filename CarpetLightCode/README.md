...
| 2f | Audio: auto-peak enable | 5 | Encoder, right = on, left = off (see "Auto-peak" below) | Front-edge neo split: left half lit = off, right half lit = on |
...
- **AGC (auto-gain) modes** (`AGCMode`: `AGCoff`/`AGCband`/`AGCfull`, persisted, default **AGCband**, subsetting 2d, encoder-cycled): decides how `normal` is boosted from floored `raw`.
  - **Off** — no gain.
  ...
  - `getBandNormalPercentPreview(band, mode)` recomputes `normal` under an explicitly-passed mode instead of the committed one — lets the AGC-mode config screen preview a live, not-yet-committed mode choice without needing to mutate (and later revert) the real committed mode.
- **Auto-peak** (persisted, default **off**, subsetting 2f — pending a Phase 2 redefinition, unchanged for now): not a loudness-scaled threshold — a per-bin hit-rate debounce on the plain threshold. Per bin: a re-crossing within 50ms of that bin's last *accepted* hit is only accepted if the previous hit has already begun decaying (dropped below its 100% hold); otherwise the effective threshold is raised out of reach for that instant. Only gates a **fresh edge** — an already-accepted, still-above-threshold hit is never interrupted, so one sustained hit's full envelope (hold/ease/sustain above) is untouched. A bin's acceptance history older than 2s is ignored (also guards the first-ever hit against a false debounce).
...
- **Unified audio history, and the audioReadTime/lightsShowTime split** (`AudioBoard.h`'s big restructure — the actual mechanism behind foresight and prediction below): every poll writes one row into a single ring buffer (`audioData_[levelType][bin][slot]` + a shared `audioTimestamps_[slot]`, sized to hold `AGC_WINDOW_MS` (4s) *plus* the max foresight setting (700ms) of history at once — the sum, not just the max of the two, since AGC's own trailing window needs to still be fully intact even at `lightsShowTime`'s offset position). Two pointers into that same buffer:
  ...
  - **`lightsShowTime`** — recomputed every poll as the row nearest to `audioReadTime`'s timestamp minus the live foresight setting. **Every external getter (`raw`/`normal`/`hit`/`RMS`, all of them) reads only from this pointer.** At 0ms foresight this equals `audioReadTime`, which is exactly why foresight-off behavior is unchanged by any of this.
...
- **AGCband is the default** at first boot; it, auto-peak, noise floor (default **18%**), peak threshold (default **68%**), sound reactivity, hit decay rate, and audio foresight all persist (`Nvm.h`).
